...
public class GetLocation extends Global {
	...
	public GetLocation() throws FileNotFoundException, IOException, ParseException {
		...
		for (rowNum = 1; rowNum <= tdSheet.getLastRowNum(); rowNum++) {
			...
			if ("Yes".equalsIgnoreCase(executionFlag)) {
				try {
					...
					response = restServices.getMethod(baseURI, resourcePath, contentType, userName, password,
							inputPayload);
			        ...
			        System.out.println( mapLocation.get("address"));
