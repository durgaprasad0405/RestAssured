...
public class DeleteLocation extends Global {
	...
	public  DeleteLocation() throws FileNotFoundException, IOException, ParseException {
		...
		for (rowNum = 1; rowNum <= tdSheet.getLastRowNum(); rowNum++) {
			...
			if ("Yes".equalsIgnoreCase(executionFlag)) {
				try {
					...
					response = restServices.deleteMethod(baseURI, resourcePath, contentType, userName, password,
							inputPayload);
			        ...
			        System.out.println( mapLocation.containsValue("OK"));
