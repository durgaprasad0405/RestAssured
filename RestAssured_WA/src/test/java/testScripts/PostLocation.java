...
public class PostLocation extends Global {
	...
	public PostLocation() throws FileNotFoundException, IOException, ParseException {
		...
		for (rowNum = 1; rowNum <= tdSheet.getLastRowNum(); rowNum++) {
			...
			if ("Yes".equalsIgnoreCase(executionFlag)) {
				try {
					...
					placeId = jsonResultMap.get("place_id").toString();
					...
					if ("Yes".equalsIgnoreCase(inputPayloadElementsMap.get("IsGetRequired?"))) 
						...
						cn = excelOps.getColNumByColName(excelFileName, sheetName, "payload_place_id");
						rn = excelOps.getRowNumByCelVal(excelFileName, sheetName, testCaseNum);
						excelOps.setCellValue(excelFileName, sheetName, rn, cn, placeId);
